...
#                'reporting_manager': _reporting_manager,
    ...
    def create(self, cr, uid, values, context=None):
        opp_info_renewal,stage_name,stage_id,delayed_app_date,postal_code='','','','',''
...
#        datetime.date.today() + datetime.timedelta(6*365/12)).isoformat()
	if values.get('postal_code',False):
            postal_code=values.get('postal_code',False)
            if '-' in postal_code:

                postal_code=postal_code.replace('-','')
                values.update({'postal_code':postal_code})
...
#        req = urllib2.Request('http://107.23.130.227/visdom/tmp/generaterecord.php?view=mortgageapplication&id=%s&username=%s&password=%s'%(ids[0],uname,pwd))
...
#        template_id = ir_model_data.get_object_reference(cr, uid, 'syml', 'email_template_related_docs')[1]
    ...
    def write(self, cr, uid, ids, vals, context=None):
        ...
        if not isinstance(ids,list):
           ids = [ids]
        stage_name,opp_info_renewal,stage_id,assig_to,user,user_id,delayed_app_date,postal_code='','','',uid,'',uid,'',''
	...
	if vals.get('postal_code',False):
            postal_code=vals.get('postal_code',False)
            if '-' in postal_code:

                postal_code=postal_code.replace('-','')
                vals.update({'postal_code':postal_code})
	    ...
	    if charge_on_title:
            ...
            if what_is_your_lending_goal and app_brow_ids:
                if what_is_your_lending_goal == '1':
                    sub_name=app_objs.applicant_name + " " + app_objs.applicant_last_name+ " - "+"Pre"+" - " +(considered_cites or "NA" )+ ", " +(application_date or 'NA')
                    vals['name']=sub_name
                elif what_is_your_lending_goal == "2":
                    sub_name=app_objs.applicant_name + " " + app_objs.applicant_last_name+" - "+"Pur"+" - "+(address or "NA")+ ", " +(city or "NA")+ ", " +(charge_on_title or "NA")+ ", " +(application_date or "NA")
                    vals['name']=sub_name
                elif what_is_your_lending_goal == "3":
                    sub_name=app_objs.applicant_name + " " + app_objs.applicant_last_name+" - "+"Ref"+" - "+(address or "NA")+ ", " +(city or "NA")+ "," +(charge_on_title or "NA")+ ", " +(application_date or "NA")
                    vals['name']=sub_name
...
#            closing_date=datetime.datetime.strptime(datetime.datetime.strptime(vals.get('closing_date',False),"%Y-%m-%d %H:%M:%S").strftime('%Y-%m-%d %H:%M:%S'), DATETIME_FORMAT)
...
#                    self.xml_set_attribute(cr,uid,customer_data[0],'annualCondoFees',annualCondoFees)
    ...
    def send_morweb_request_test(self,cr,uid,ids,context=None):
        ...
        if morweb_request:
            ...
            netRate= crm_browse.rate or 00.00
            property_value = crm_browse.property_value or 00.00
#            create_day=create_date[:10].strip()
            fundingDate = crm_browse.expected_closing_date or ''
            if fundingDate:
                fundingDate =fundingDate[:10].strip()
            mortgageapp = '''<MortgageApplication agentUserId="0" loanPurposeType="3" purchasePriceEstimatedValue="%s" fundingDate="%s">'''%(property_value,fundingDate)
            loanAmount = crm_browse.total_mortgage_amount or 00.00
            loanAmount  = '''<Loan loanAmount="%s" netRate="%s" chargeType="1"/>'''%(loanAmount,netRate)
            ...
            if script_response.find('<ProcessRequestResponse'):
                ...
                if responseDOM1.getElementsByTagName('BDIResponse'):
                    ...
                    if applicationnumber:
                        self.write(cr,uid,ids,{'morweb_filogix':applicationnumber,})
