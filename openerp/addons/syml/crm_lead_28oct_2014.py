...
#                second mail
                ...
                if crm_browse.condition_of_financing_date:
                    ...
                    from_dt=(crm_browse.condition_of_financing_date)
                    from_dt=datetime.datetime.strptime(datetime.datetime.strptime(from_dt,'%Y-%m-%d %H:%M:%S').strftime('%Y-%m-%d %H:%M:%S'), DATETIME_FORMAT)
                    from_dt = from_dt + datetime.timedelta(hours=5 , minutes=30)
                    comp_app_name=' Send Condition of Financing Letter before' + "  " + (str(from_dt) or '') + "  " + \
...
#                    context.update({'proposal':ids[0],'user_id':uid})
            ...
            if stage_brw.name == 'Commitment':
                ...
                project_task.create(cr, uid, {
                            #'name': '%s: Task for %s %s' % (cur_obj.name or '', cur_obj.name2 , cur_obj.name3),
                            #'name': 'Contact' + ' ' + app_name + ' ' +   ' with a phone call before ' +  ' ' + deadline + 'to review the proposal together and answer questions / have product selected.',
...
#                        self.pool.get('email.template').send_mail(cr,uid,template_id_file_comp_ref,ids[0],'True',context)
			...
			print"template_no_traileeeeeeeeeeeeeeeeeeeeeeeeeeeeeeer",template_id_no_trailer
...
#                    if crm_browse.partner_id:
	...
	if vals.get('what_is_your_lending_goal',False) and app_brow_ids:
            what_is_your_lending_goal,considered_cites,application_date,charge_on_title,address,city='','','','','',''
            if app_brow_ids:
                app_objs=app_brow_ids[0]
#            if vals.has_key('what_is_your_lending_goal'):
            what_is_your_lending_goal=vals.get('what_is_your_lending_goal')
    #        else:
